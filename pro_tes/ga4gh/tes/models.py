...
class TesExecutor(BaseModel):
    ...
    command: List[str] = Field(
        [''],
        description='A sequence of program arguments to execute, where the \
    ...
    workdir: Optional[str] = Field(
        None,
        description='The working directory that the command will be executed \
            in.\nIf not defined, the system will default to the directory set\
                by\nthe container image.',
        example='/data/',
    )
    stdin: Optional[str] = Field(
        None,
        description='Path inside the container to a file which will be \
            piped\nto the executor\'s stdin. This must be an absolute path.\
            This mechanism\ncould be used in conjunction with the input \
            declaration to process\na data file using a tool that expects\
            STDIN.\n\nFor example, to get the MD5 sum of a file by reading \
            it into the STDIN\n```\n{\n  "command" : ["/bin/md5"],\n  "stdin" \
                : "/data/file1"\n}\n```',
        example='/data/file1',
    )
    stdout: Optional[str] = Field(
        None,
        description='Path inside the container to a file where the \
...
class TesExecutorLog(BaseModel):
    ...
    stdout: Optional[str] = Field(
        ...
        description='Stdout content.\n\nThis is meant for convenience. No\
            ...
            different approaches: only the head, only the tail,\na URL \
            reference only, etc.\n\nIn order to capture the full stdout \
            client should set Executor.stdout\nto a container file path,\
            and use Task.outputs to upload that file\nto permanent storage.',
    )
    stderr: Optional[str] = Field(
        None,
        description='Stderr content.\n\nThis is meant for convenience. No \
            guarantees are made about the content.\nImplementations may chose \
            different approaches: only the head, only the tail,\na URL \
            reference only, etc.\n\nIn order to capture the full stderr\
            client should set Executor.stderr\nto a container file path,\
            and use Task.outputs to upload that file\nto permanent storage.',
    )
...
class TesInput(BaseModel):
    ...
    content: Optional[str] = Field(
        ...
        description='File content literal.\n\nImplementations should support a\
            ...
            maximum.\n\nUTF-8 encoded\n\nIf content is not empty, "url" \
            must be ignored.',
    )


class TesOutput(BaseModel):
    name: Optional[str] = Field(None, description='User-provided name of \
        output file')
    description: Optional[str] = Field(
        None,
        description='Optional users provided description field, can be used \
            for documentation.',
    )
    url: str = Field(
        ...,
...
class TesOutputFileLog(BaseModel):
    url: str = Field(
        ..., description='URL of the file in storage, \
            e.g. s3://bucket/file.txt'
    )
    path: str = Field(
        ...,
        description='Path of the file inside the container. Must be an \
            absolute path.',
    )
    size_bytes: str = Field(
        ...,
        description="Size of the file in bytes. Note, this is currently coded \
...
class TesResources(BaseModel):
    ...
    preemptible: Optional[bool] = Field(
        None,
        description="Define if the task is allowed to run on preemptible \
            compute instances,\nfor example, AWS Spot. This option may have no\
            effect when utilized\non some backends that don't have the \
            concept of preemptible jobs.",
        example=False,
    )
    ...
    zones: Optional[List[str]] = Field(
        None,
        description='Request that the task be run in these compute zones. How\
            this string\nis utilized will be dependent on the backend system.\
            For example, a\nsystem based on a cluster queueing system may use \
            this string to define\npriorty queue to which the job is\
                assigned.',
        example='us-west-1',
    )
...
class ServiceType(BaseModel):
    group: str = Field(
        ...,
        description="Namespace in reverse domain name format. Use `org.ga4gh` \
            for implementations compliant with official GA4GH specifications.\
            For services with custom APIs not standardized by GA4GH, or \
            implementations diverging from official GA4GH specifications,\
            use a different namespace (e.g. your organization's reverse domain\
                name).",
        example='org.ga4gh',
    )
    artifact: str = Field(
...
class Service(BaseModel):
    id: str = Field(
        ...,
        description='Unique ID of this service. Reverse domain name notation \
            is recommended, though not required. The identifier should attempt\
            to be globally unique so it can be used in downstream aggregator \
    ...
    description: Optional[str] = Field(
        None,
        description='Description of the service. Should be human readable and \
    ...
    createdAt: Optional[datetime] = Field(
        None,
        description='Timestamp describing when the service was first deployed\
    ...
    environment: Optional[str] = Field(
        None,
        description='Environment the service is running in. Use this to \
    ...
    version: str = Field(
        ...,
        description='Version of the service being described. Semantic\
            versioning is recommended, but other identifiers, such as dates or\
                commit hashes, are also allowed. The version should be changed\
...
class TesTaskLog(BaseModel):
    logs: List[TesExecutorLog] = Field(..., description='Logs for each \
        executor')
    metadata: Optional[Dict[str, str]] = Field(
        None,
        description='Arbitrary logging metadata included by the \
            implementation.',
    ...
    system_logs: Optional[List[str]] = Field(
        ...
        description='System logs are any logs the system decides are relevant,\
            ...
            implementation specific: format, size, etc.\n\nSystem logs may \
            be collected here to provide convenient access.\n\nFor \
            example, the system may include the name of the host\\nwhere\
...
class TesServiceInfo(Service):
    storage: Optional[List[str]] = Field(
        None,
        description='Lists some, but not necessarily all, storage locations \
            supported\nby the service.',
        example=[
...
class TesTask(BaseModel):
    ...
    description: Optional[str] = Field(
        None,
        description='Optional user-provided description of task for\
            documentation purposes.',
    )
    inputs: Optional[List[TesInput]] = Field(
        None,
        description='Input files that will be used by the task. Inputs will be\
            downloaded\nand mounted into the executor container as defined by \
                the task request\ndocument.',
    ...
    executors: List[TesExecutor] = Field(
        [TesExecutor],
        description='An array of executors to be run. Each of the executors\
            will run one\nat a time sequentially. Each executor is a different\
            command that\nwill be run, and each can utilize a different docker\
            image. But each of\nthe executors will see the same mapped inputs \
            and volumes that are declared\nin the parent CreateTask \
            message.\n\nExecution stops on the first error.',
    )
    volumes: Optional[List[str]] = Field(
        None,
        description='Volumes are directories which may be used to share data\
            between\nExecutors. Volumes are initialized as empty directories \
            by the\nsystem when the task starts and are mounted at the same \
            path\nin each Executor.\n\nFor example, given a volume defined at \
            `/vol/A`,\nexecutor 1 may write a file to `/vol/A/exec1.out.txt`, \
            then \n executor 2 may read from that file.\n\n(Essentially, this \
            translates to a `docker run -v` flag where\nthe container path is \
            the same for each executor).',
        example=['/vol/A/'],
    )
    tags: Optional[Dict[str, str]] = Field(
        None,
        description='A key-value map of arbitrary tags. These can be used to \
...
class TesListTasksResponse(BaseModel):
    tasks: List[TesTask] = Field(
        ...
        description='List of tasks. These tasks will be based on the original \
            submitted\ntask document, but with other fields, such as the job \
...
class DbDocument(BaseModel):
    """Model for task request database document.

    Args:
        task_log: Complete logging information for task.
