...
tasks_body = {"executors": [{"image": "alpine", "command": ["echo", "hello"]}]}
...
def test_get_tasks_minimal_200():
    ...
    params = {"view": "MINIMAL"}
    response = requests.get(url=f"{tes_url}/tasks", params=params)
...
def test_get_task_by_id_minimal():
    ...
    response = requests.get(
        url=f"{tes_url}/tasks/{id[0]}", params={"view": "MINIMAL"}
    )
    assert response.status_code == 200
    assert response.json()["id"] == id[0]
...
def test_get_task_by_id_basic():
    ...
    id = post_response.json()["id"]
    response = requests.get(
        url=f"{tes_url}/tasks/{id}", params={"view": "BASIC"}
    )
    assert response.status_code == 200
...
def test_get_task_by_id_full():
    ...
    id = post_response.json()["id"]
    response = requests.get(
        url=f"{tes_url}/tasks/{id}", params={"view": "FULL"}
    )
    assert response.status_code == 200
    assert response.json()["id"] == id
    assert response.json()["state"]
...
def test_get_service_info_200():
    response = requests.get(url=f"{tes_url}/service-info")
    assert response.status_code == 200
...
def test_cancel_task_200():
    ...
    assert response.status_code == 200
    # may need to wait long for next test as task may be stay in queue
    # and may still fail for no fault of ours
    # for _ in range(60):
    #     response_status = requests.get(
    #         url=f"{tes_url}/tasks/{id}", params={"view": "MINIMAL"}
    #     )
    #     sleep(5)
    #     if response_status.json()["state"] == "CANCELED":
    #         break
    # assert response_status.json()["state"] == "CANCELED"
